...
class Ship {
    ...
    // Pass +1 to rotate clockwise, -1 to rotate counter clockwise
    Rotate(dir) {
        this.angle += this.rotateSpeed * dir;
    }

    Update() {
        let radians = this.angle / Math.PI * 180; // Convert degrees to radians
        ...
        // Move position to opposite side of the canvas 
        // if the coordinates go beyond the canvas
...
class Asteroid {
    ...
    Draw() {
        ...
        let vertAngle = ((Math.PI * 2) / numberOfSides)
        let radians = this.angle / Math.PI * 180;
...
function Render() {
    ...
    if (asteroids.length !== 0) {
        for (let k = 0; k < asteroids.length; k++) {
            if (CircleCollision(ship.x, ship.y, 11, 
